...
object RowByRowAnalysis {
  ...
  /** Recursively generates a JSON string representing the differences between two DataFrames for a given row.
    *
    * @param maskedColumns
    *   The columns to compare.
    * @param maskedA
    *   masked row A to compare.
    * @param maskedB
    *   masked row B to compare against.
    * @param res
    *   The accumulated result string.
    * @return
    *   The JSON string representing the differences for the given row.
    */
  @tailrec
  private def getDiff(
      maskedColumns: Array[String],
      maskedA: Seq[Any],
      maskedB: Seq[Any],
      res: List[ColumnsDiff] = List()
  ): List[ColumnsDiff] = {
    if (maskedColumns.isEmpty) {
      ...
      if (columnName == HASH_COLUMN_NAME) getDiff(maskedColumns.tail, maskedA.tail, maskedB.tail, res)
        ...
        getDiff(
          ...
          maskedA.tail,
          maskedB.tail,
          res :+ ColumnsDiff(columnName = columnName, values = List(a.toString, b.toString))
        )
      }
    }
  }

  /** Apply the mask that was created by getBest to the columns of the DataFrame and rows A and B. By applying the mask
    * we will pick only the values that are different.
    * @param diffA
    *   The DataFrame A to compare.
    * @param rowA
    *   The current row in DataFrame A.
    * @param rowB
    *   The current row in DataFrame B.
    * @param mask
    *   The mask to apply.
    * @return
  ...
  private def getMasked(diffA: DataFrame, rowA: Row, rowB: Row, mask: Seq[Int]): (Array[String], Seq[Any], Seq[Any]) = {
    val maskedColumns = diffA.columns.zip(mask).collect { case (col, 1) =>
      col
    }
    val maskedA = rowA.toSeq.zip(mask).collect { case (col, 1) =>
      col
    }
    val maskedB = rowB.toSeq.zip(mask).collect { case (col, 1) =>
      col
    }
  ...
  /** Recursively generates a JSON string representing the differences between two DataFrames.
    ...
    *   The accumulated result string.
    * @return
    *   The List[RowsDiff] representing the differences.
    */
  @tailrec
  private def generateDiffJson(
    ...
    val diffForRow = RowsDiff(
      inputAHash = hashA.toString,
      inputBHash = hashB.toString,
      diffs = getDiff(maskedColumns, maskedA, maskedB)
    )
  ...
  /** Analyse the differences between two DataFrames
    ...
    *   The DataFrame B to compare against.
    * @return
    *   List[RowsDiff] containing the differences between the two DataFrames A to B.
    */
