 ...
 * @param bestScore represents score of difference between two rows. It is picked the best one, teh lowest score.
 *                  For every difference in rows it is increased by 1, so best score is 1 for only one difference,
