...
object HashDataFrame {

  private val logger: Logger = LoggerFactory.getLogger(HashDataFrame.getClass)
   ...
   */
  def hash(df: DataFrame): DataFrame = {
    logger.info("Hashing the dataframe")
    df.withColumn(Main.HashName , hashUDF(struct(df.columns.map(col): _*)))
  }
